...
def validate(exp_dir, device, model, val_dataloader, epoch, viz=False):
    ...
    for data in val_dataloader:
        ...
        selected_ts = selected_ts.transpose(1, 0).to(device)
        rel_ts = rel_ts.transpose(1, 0).float().to(device)
...
def train(opt, exp_dir, model, train_dataloader, val_dataloader):
    ...
    for epoch in range(opt.epochs):
        ...
        for step, data in enumerate(train_dataloader):
            ...
            inp_frames, selected_ts, rel_target_ts, target_t, target_coord, target_rgb, cell = \
                data['inp_frames'], data['selected_ts'], data['rel_target_ts'], data['target_t'], \
                data['target_coord'], data['target_rgb'], data['cell']
            selected_ts, rel_target_ts = selected_ts.view(-1, 1), rel_target_ts.view(-1, 1)

            pred_frame, warped, visibilities = model(inp_frames, target_coord, cell, selected_ts, rel_target_ts)
