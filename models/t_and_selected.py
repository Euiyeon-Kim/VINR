...
class VINR(nn.Module):
    ...
    def get_rgb(self, frames, feat, query_coord, cell, inp_ts, t):
        ...
        for inp_t in inp_ts.permute(1, 0):
            t_and_selected = torch.concat([torch.unsqueeze(inp_t, -1), t], dim=-1)
            fx = self.flow_generator(t_and_selected, mod_params)
            flows.append(fx)
...
def make_liif_flow(common_opt, specified_opt):
    ...
    flow_generator = ModGenerator(in_dim=2, out_dim=2, w0=specified_opt.w0,
