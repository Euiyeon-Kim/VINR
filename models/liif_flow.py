...
class VINR(nn.Module):
    def __init__(self, encoder, liif, mapper, num_frames, device):
    ...
    def forward(self, frames, query_coord, cell, t):
        ...
        pred = self.get_rgb(frames, feat, query_coord, cell, t)
        return pred
    ...
    def get_rgb(self, frames, feat, query_coord, cell, t):
        B, C, F, H, W = frames.shape

        mod_params = self.liif(feat, query_coord, cell)
        flows_and_visibilities = self.mapper(t, mod_params)
        flows = flows_and_visibilities[:, :, :self.num_frames * 2]
        visibilities = torch.softmax(flows_and_visibilities[:, :, self.num_frames * 2:], -1)
        flows = flows.contiguous().view(B, H, W, -1)
        visibilities = visibilities.contiguous().view(B, H, W, -1)
...
def make_liif_flow(common_opt, specified_opt):
    ...
    mapper = ModRGBMapper(out_dim=3*common_opt.num_frames, w0=specified_opt.w0,
                          hidden_node=specified_opt.hidden, depth=specified_opt.depth)
    vinr = VINR(encoder, liif, mapper, common_opt.num_frames, device)
